  ...
  <div class="homepage">
    <v-row>
            ...
            <v-card-text class="pa-0 pa-sm-6">
              ...
              <v-expand-transition>
                <div v-show="searchExpand">
                  <div class="mt-2">
                    <div class="subtitle">年级</div>
                    <v-row>
                      <v-col class="py-0" cols="6" v-for="level in levels">
                        <v-checkbox
                          :label="level.title"
                          :value="level.value"
                          hide-details
                        ...
                        >
                          <template v-slot:append>
                            <v-chip color="primary" class="px-2" label small>
                              {{ activeProperties.levels[level.value] || 0 }}
                            </v-chip>
                          </template>
                        </v-checkbox>
                      </v-col>
      ...
      <v-col v-if="items().length" class="pt-0" cols="12" :sm="6" :md="3" v-for="(item, index) in items()" :key="'item-'+index">
        <router-link tag="div" class="cursor-pointer mb-2" :to="{ name: 'item', params: { uuid: item.uuid }}">
          <v-card elevation="0" class="mb-8">
            ...
            <div class="chips">
              <v-chip 
                ...
                class="mt-2"
                outlined
                v-for="place in item.hand_delivery.slice(0,1)" 
                :key="place">
                {{ placesMap()[place] }}
              </v-chip>
              <v-chip label small class="ml-2 mt-2" v-if="item.post_delivery.length">
                可邮寄
              </v-chip>
            </div>
